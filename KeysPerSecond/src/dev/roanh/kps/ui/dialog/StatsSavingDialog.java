...
public class StatsSavingDialog extends JPanel{
	...
	private StatsSavingDialog(StatsSavingSettings config){
		...
		loadOnStart = new JCheckBox("在启动时从文件加载保存的统计信息", config.isLoadOnLaunchEnabled());
		...
		periodicPanel.setBorder(BorderFactory.createTitledBorder("定时保存"));
		enabled = new JCheckBox("定期将到目前为止的统计数据保存到文件中", config.isAutoSaveEnabled());
	...
	private void showFormatHelp(ActionEvent event){
		...
		JLabel more = new JLabel("<html><font color=blue><u>更多选项可以在DateTimeFormatter中的avadoc中找到。</u></font></html>");
