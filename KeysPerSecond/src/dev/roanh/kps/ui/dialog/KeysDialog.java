...
public class KeysDialog extends JPanel implements KeyPressListener{
	 ...
	 * @see #lastKey
	 */
	private JTextField pressed = new JTextField("<按下一个按键>", 25);
	/**
	...
	private KeysDialog(Configuration config, boolean live){
		...
		left.setBorder(BorderFactory.createTitledBorder("当前添加的按键"));
		...
		addButton.setBorder(BorderFactory.createTitledBorder("鼠标按键"));
		...
		cons.gridy = 2;
		addButton.add(newButton(3, "M3 (中键)"), cons);
		cons.gridy = 3;
		cons.gridwidth = 1;
	...
	private JButton newButton(int code, String text){
		...
		button.addActionListener(e->{
			...
			if(keySettings.contains(info)){
				Dialog.showMessageDialog("M" + code + " 按键已经添加过了。\n请勿重复添加。");
			}else{
	...
	private void commitKey(){
		if(lastKey == -1){
			Dialog.showMessageDialog("请先按下一个按键。");
			return;
		}
		
		KeyPanelSettings info = new KeyPanelSettings(placePanel(), lastKey);
		if(keySettings.contains(info)){
			Dialog.showMessageDialog("按键已经添加过了。\n请勿重复添加。");
		}else{
			keySettings.add(info);
			keys.addPanelRow(keySettings, info);
			keys.revalidate();
