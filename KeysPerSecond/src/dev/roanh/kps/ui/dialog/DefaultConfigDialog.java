...
public class DefaultConfigDialog extends JPanel{
	...
	private DefaultConfigDialog(){
		...
		add(new JLabel("您可以配置一个默认配置，在启动时自动打开。"), BorderLayout.PAGE_START);
	...
	public static final void showDefaultConfigDialog(){
		...
		try{
			switch(Dialog.showDialog(dialog, new String[]{"保存", "删除默认配置", "取消"})){
			case 0:
			...
			e.printStackTrace();
			Dialog.showErrorDialog("保存失败, 原因为: " + e.getMessage());
		}
	}
}
