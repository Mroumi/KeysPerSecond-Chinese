	...
	public static final Configuration loadConfiguration(){
		...
		try{
			...
			if(parser.wasDefaultUsed()){
				Dialog.showMessageDialog("成功加载配置，但使用了一些默认值。");
			}else{
				Dialog.showMessageDialog("配置加载成功");
			}
			
			return parser.getConfig();
		}catch(IOException e){
			e.printStackTrace();
			Dialog.showErrorDialog("配置加载失败, 原因: " + e.getMessage());
			return null;
		}
	}
	...
	public static void reloadConfig(){
		if(Main.config.getPath() != null){
			try{
				...
				e.printStackTrace();
				Dialog.showErrorDialog("未能重新加载配置, 原因: " + e.getMessage());
			}
		}else{
			Dialog.showMessageDialog("无法重新加载未从文件中加载的配置");
		}
	}
